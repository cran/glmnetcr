...
fit <- glmnet.cr(x,y)
@
As with \code{glmnet} model objects, methods such as \code{print} and \code{plot} can be applied to \code{glmnet.cr} model objects, which are helpful for selecting the step at which to select the final model from the solution path. 
<<echo=TRUE>>=
print(fit)
